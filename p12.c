...
void Dijkstra(Table T, Graph G, Vertex Start)
{
	...
	{
		U = DeleteMin(Min_Heap);// ADD EACH VERTEX IN PRIORITY QUEUE
		printf("%2d", U);
...

{
...

	{
		...
		H->Elements[i].Distance = X.Distance;
...
ElementType DeleteMin(PriorityQueue H)
{
	...
	{
		Min_Data = H->Elements[1].Distance;
		Last_data = H->Elements[H->Size--].Distance;
		for (i = 1; 2 * (i) <= H->Size; i = child)
		{
			...
			if (H->Elements[child].Distance < Last_data)
			{

				H->Elements[i].Distance = H->Elements[child].Distance;
	...
	H->Elements[i].Distance = Last_data;

	return H->Elements[i].Index;
